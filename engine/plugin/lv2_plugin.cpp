...
bool Lv2Plugin::load(const QString &uri, int) {
    ...
    for (uint32_t i = 0; i < numPorts; ++i) {
        ...
        if (lilv_port_is_a(m_plugin, port, audioClass)) {
            if (lilv_port_is_a(m_plugin, port, inputClass) && m_audioInPort < 0)
                m_audioInPort = (int)i;
            if (lilv_port_is_a(m_plugin, port, outputClass) && m_audioOutPort < 0)
...
void Lv2Plugin::process(float *buf, int frameCount) {
    if (!m_instance || m_audioInPort < 0 || m_audioOutPort < 0)
        return;
