...
Common.BaseEffect {
    ...
    property real  strength:  Math.max(0, root.evalNumber("strength",   100) / 100.0)
    property real  diffusion: Math.max(0, root.evalNumber("diffusion",   20))
    property real  threshold: root.evalNumber("threshold",  0) / 100.0
    ...
    property real  shape: {
        ...
        if (s === "cross4")     return 1.0
        if (s === "cross4diag") return 2.0
        if (s === "cross8")     return 3.0
        if (s === "lineH")      return 4.0
        if (s === "lineV")      return 5.0
        return 0.0
    }
    ...
    ShaderEffect {
        ...
        property real    glowOnly:   root.glowOnly ? 1.0 : 0.0
        anchors.fill: parent
