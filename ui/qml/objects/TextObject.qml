...
Common.BaseObject {
    ...
    Model {
        source: "#Rectangle"
        scale: Qt.vector3d(
            (renderer.output.sourceItem ? renderer.output.sourceItem.width  : 1) / 100,
            (renderer.output.sourceItem ? renderer.output.sourceItem.height : 1) / 100,
            1)
        ...
        Item {
            ...
            Canvas {
                ...
                Connections {
                    ...
                    function onOutlineColorChanged()  { outlineCanvas.requestPaint() }
                    function onOutlineEnabledChanged(){ outlineCanvas.requestPaint() }
                    function onTextContentChanged()   { outlineCanvas.requestPaint() }
                    function onFontFamilyChanged()    { outlineCanvas.requestPaint() }
                    function onFontSizeChanged()      { outlineCanvas.requestPaint() }
                    function onFontBoldChanged()      { outlineCanvas.requestPaint() }
                    function onFontItalicChanged()    { outlineCanvas.requestPaint() }
                    function onAlignmentChanged()     { outlineCanvas.requestPaint() }
                    function onLineSpacingChanged()   { outlineCanvas.requestPaint() }
                }
                ...
                onPaint: {
                    ...
                    if (!root.outlineEnabled || root.outlineWidth <= 0) return;

                    var wt  = root.fontBold   ? "bold"   : "normal";
                    var st  = root.fontItalic ? "italic" : "normal";
                    var lsp = root.letterSpacing !== 0
                              ? " letter-spacing: " + root.letterSpacing + "px;" : "";
                    ctx2d.font        = st + " " + wt + " " + root.fontSize + "px '"
                                        + root.fontFamily + "'";
                    ...
                    var alignStr;
                    if      (root.alignment === Text.AlignLeft)  alignStr = "left";
                    else if (root.alignment === Text.AlignRight) alignStr = "right";
                    else                                         alignStr = "center";
                    ctx2d.textAlign    = alignStr;
                    ctx2d.textBaseline = "alphabetic";

                    var xPos = (alignStr === "left")  ? root._pad
                             : (alignStr === "right") ? width - root._pad
                             :                          width / 2;
            ...
            Text {
                ...
                lineHeightMode:   root.lineSpacing > 0 ? Text.FixedHeight
